...
class Encoder(nn.Module):
    ...
    def __init__(self, img_scale):
        ...
        self.current_size = ceil(((((img_scale - 5)/4 + 1) - 3)/4 + 1))
        ...
        self.encoder_cnn = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=3, stride=4, padding=0, ceil_mode=True),
        )
        ...
        self.encoder_lin = nn.Sequential(
            ...
            nn.Linear(in_features=64 * self.current_size * self.current_size,
                      out_features=64 * self.current_size),
            nn.LeakyReLU(negative_slope=0.3),

            # 2nd fully connected layer
            nn.Linear(in_features=64 * self.current_size, out_features=128),
            nn.LeakyReLU(negative_slope=0.3),
